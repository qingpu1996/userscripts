...
## 当前策略数据边界（2026-07-14 重置）

- 唯一盲玩限制：不得搜索、读取、导入或使用《魔塔24层》的攻略、标准路线、通关录像及其他人为整理的针对性解法资料。
- 游戏自身数据全部可读：完整运行态、`core.floors`、完整/未来 maps、`material`、物品/怪物/事件定义、地图源码和存档结构都可用于策略分析。
- 游戏定义与实时状态是资源、怪物和机制语义的权威；不得继续为 yellowKey 等对象维护逐 ID 硬编码事实库。
- 读取权限不包含写权限：禁止直接篡改 hero、地图/block、怪物、事件、存档或其他游戏权威对象；所有游戏变化必须通过正常引擎接口并执行 guard、稳定与真实差分校验。
- 运行通信保持 localhost-only；读取完整游戏数据不授权向非本地端点发送工程、运行态或存档。

### 本轮策略边界重置任务

- [x] **F300 撤销旧读取白名单**：当前文档、配置、测试和静态门禁不再禁止完整 runtime/source definitions 只读分析。
- [x] **F301 改造静态门禁**：Acorn 扫描只拦截游戏权威对象直接 mutation，并保留 localhost 构建边界。
- [x] **F302 正反 fixture**：完整 floors/maps/material/source/save structure 读取通过；hero/map/block/monster/event/save 直接写入、删除和 Object/Reflect 绕过失败。
- [x] **F303 完整离线 QA**：`113 JS + 114 Python + 1 integration = 228/228`；协议/schema、compile/syntax、双 dist 确定性、Static runtime safety、docs/JSON、diff 与隔离 prospective index 全绿，证据位于 `qa/runs/2026-07-14-remove-blind-data-boundary/`。
- [x] **F304 Fix 6 静态 lint 定位纠正**：Acorn 分析明确为受控项目源码支持子集与防回归 lint，不再声称证明任意 JavaScript/alias/closure/this/constructor/callback 语义；unsupported identity 构造显式 fail closed。
- [x] **F305 实际 production 完整性审计**：独立扫描全部 src/service/双 dist，固定 adapter 权威 alias 清单与 read/action API inventory；未分类 engine API、实际权威写、delete/defineProperty/Reflect/container mutator 均失败。
- [x] **F306 动态写入 instrumentation**：full-cycle fake core 代理 hero/maps/blocks/enemy 权威对象，collector/client/service 阶段零写；所有真实差分只允许出现在 `moveDirectly`/`setAutomaticRoute` 模拟公开 API 作用域。
- [x] **F307 Fix 6 QA 证据**：方向说明、actual-source 审计清单、定向 AST/integration、`127 JS + 116 Python + 1 integration = 244/244` 完整离线 QA、双 dist hash 与 diff 记录归档到 `qa/runs/2026-07-14-remove-blind-data-boundary-fix6/`。
- [ ] **F308 全新只读验收**：由主会话在开发 Agent 结束后派发，开发 Agent 不得自验替代。
...
- [x] **V231 物理遍历扫描（历史实现）**：当时从 current observation 和真实 change-map 扩展；F300 后允许用完整游戏定义预建模，但真实执行仍逐边界核对 transition。
...
- [x] **F214 结构化静态合规（历史）**：当时覆盖 runtime member-chain 与 hero status 直接写；其中旧 floors 读取禁令已由 F300 撤销，src/service/双 dist 现统一执行运行态写安全门禁。
...
- [x] **F260 完整离线 QA 与双构建证据**：`104 JS + 91 Python + 1 integration = 196/196`；协议/schema/compile、双 dist 确定性、当时的 Acorn 静态策略、docs/JSON、diff 和隔离 prospective staged 检查全绿，证据位于 `qa/runs/2026-07-14-live-runtime-authority/`；旧读取禁令已由 F300 撤销。
...
- [x] **F266 完整离线 QA 与证据**：`104 JS + 97 Python + 1 integration = 202/202`；双 dist hash、当时的静态策略、协议/schema、compile/syntax、docs/JSON、diff 与隔离 prospective index 全绿，证据位于 `qa/runs/2026-07-14-optional-unsupported-frontier/`；旧读取禁令已由 F300 撤销。
...
- [x] **F271 完整离线 QA 与证据**：定向红灯、完整 JS/Python/integration、协议/schema、compile/syntax、双 dist 确定性、当时的 Acorn 静态策略、docs/JSON、diff 与隔离 prospective index 全绿；证据位于 `qa/runs/2026-07-14-zero-key-omission-recovery/`；旧读取禁令已由 F300 撤销。
...
- [x] **F278 完整离线 QA 与证据**：`108 JS + 103 Python + 1 integration = 212/212`；协议/schema、compile/syntax、双 dist 确定性、当时的 Acorn 静态策略、docs/JSON、diff 与隔离 prospective index 全绿，证据位于 `qa/runs/2026-07-14-live-enemy-semantics/`；旧读取禁令已由 F300 撤销。
...
- [x] **F283 恢复与执行安全回归**：known_unfightable、真正未知 damage、takeover scan、MT0→MT1 completed/ack、浏览器和服务重启零重放、单边界与当时的静态策略门禁继续通过；读取范围已由 F300 重置。
- [x] **F284 完整离线 QA 与证据**：`108 JS + 107 Python + 1 integration = 216/216`；协议/schema、compile/syntax、双 dist 确定性、当时的 Acorn 静态策略、docs/JSON、diff 与隔离 prospective index 全绿，证据位于 `qa/runs/2026-07-14-live-enemy-root-only-followup/`；旧读取禁令已由 F300 撤销。
...
- [x] **F289 完整离线 QA 与证据**：`111 JS + 108 Python + 1 integration = 220/220`；协议/schema、compile/syntax、双 dist 确定性、当时的 Acorn 静态策略、docs/JSON、diff 与隔离 prospective index 全绿，证据位于 `qa/runs/2026-07-14-damage-undefined-fail-closed/`；旧读取禁令已由 F300 撤销。
...
- [x] **F296 完整离线 QA 与证据**：`111 JS + 114 Python + 1 integration = 226/226`；protocol/schema、compile/syntax、双 dist 确定性、当时的 Acorn 静态策略、docs/JSON、diff 与隔离 prospective index 全绿，证据位于 `qa/runs/2026-07-14-verified-stair-cycle/`；旧读取禁令已由 F300 撤销。
...
交付一套“浏览器运行态代理 + localhost 决策服务”，在游戏数据可完整读取、执行状态不可直接篡改的边界内完成以下闭环：

1. 浏览器端可读取完整游戏运行态、地图、素材、定义和存档结构，通用解析资源与机制语义。
2. 脚本将当前执行现场序列化为 Protocol observation，完整定义作为本地策略分析输入。
3. 本地服务可结合游戏定义、未来地图和实时状态建立模型并返回原子行动。
4. 脚本在执行前重新核对 guard，通过游戏正常公开接口执行行动。
5. 每个状态变化边界后重新读取真实状态，等待稳定，校验预期差分并回报。
6. 游戏定义可通用解析的新对象/楼层不暂停；只在定义解析失败、交互未实现、服务不可用或预测不符时暂停。
7. 刷新、超时、丢包和服务重启时不盲目重放同一个行动。
8. 最终生成可直接安装的 `dist/mota-planning-lab.user.js`，但不自动安装、不自动发布、不自动覆盖存档。

项目完成不等于“脚本能够点击几步”，而是外部攻略隔离、行动原子性、恢复幂等、差分校验、暂停取证、决策日志和自动化测试全部通过。

## 2. 本轮开发明确不做

- 不访问目标游戏页面。
- 不读取任何当前或历史游戏运行态。
- 不检索《魔塔24层》的攻略、标准路线、通关录像或其他人为整理的针对性解法资料；游戏自身工程文件和运行态允许读取。
...
- `engine-adapter` 是页面运行态集中入口，但不是读取白名单。
- 允许读取 `core`、完整 `core.status`、`core.floors`、所有 maps、`material`、物品/怪物/事件定义、地图源码、完整初始地图和存档结构。
- 可读取未到达楼层和同层多个 map 的定义，提前建立地图、资源与机制模型。
- 资源与机制必须优先从游戏权威定义通用解析，不再按 yellowKey 等具体 ID 逐项适配。
- 唯一禁止的数据源是外部攻略、标准路线、通关录像和其他人为整理的针对性解法资料。

### 3.2 本地通信

- 网络请求只允许发往 `http://127.0.0.1:18724/cycle`。
- Protocol v2 当前 `/cycle` 请求仍按 schema 发送执行现场、上一行动完成标识和恢复元数据；完整游戏数据可在本机另行解析为策略输入。
- 读取完整游戏数据不授权发送到非本地端点；日志不得包含 Cookie、登录信息，也不应无意复制整个游戏工程或个人存档。

### 3.3 行动完整性

- 禁止直接写入、删除或通过 Object/Reflect 绕过修改 hero 数值、地图/block、怪物、事件、存档或其他游戏权威对象。
...
5. 启动后先读取游戏完整定义建立模型；只有楼层、block 或机制无法通用解析时生成可打标暂停包。
...
H5 魔塔当前页面
    │
    │ 游戏自身 source / definitions / full runtime / save structures（只读）
    ▼
Tampermonkey 运行态代理
    ├─ engine-adapter：唯一 core 读取与正常行动接口边界
    ├─ observer：白名单观察序列化
    ├─ guard / fingerprint / delta：执行前后校验
    ├─ executor / stability：单边界执行与稳定等待
    ├─ journal / recovery：持久幂等与丢包恢复
    ├─ localhost-client：按决策所需发送游戏自身数据，不引入外部攻略
...
- 读取完整游戏运行态、floors/maps/material、物品/怪物/事件定义和存档结构，生成只读策略输入。
- 对当前执行现场额外同步采集 floorId、hero、当前 map/blocks 和逐坐标实时战损，用于 guard 与结算。
...
其他模块优先接收 adapter 返回的不可变普通对象，避免策略分析代码意外持有可写页面引用；这不是读取范围限制。
...
- `floor_name` 可从当前 map、楼层全集或当前可见 HUD 获得，并记录来源。
...
- 怪物定义可从游戏完整素材读取；实际执行前的战损和动态属性仍以 fresh runtime 为结算依据。
- 战损为 `null`、`"???"`、非有限数或无法解释时暂停。
- `busy` 仅由移动、锁控制和活动事件布尔状态派生。
- `captured_at` 使用毫秒 Unix 时间。

Protocol observation 按 schema 发送当前执行现场；完整运行态与源码定义作为本机策略数据，不得无意发往非 localhost 端点。
...
任何无法从游戏自身定义通用解析、且本地也无可验证标签的组合，均以 `pause_kind=NEW_OBJECT_OR_MECHANISM`、`detail_code=UNKNOWN_BLOCK` 暂停，并保存当前层完整观察和以下定位信息：
...
- 限制请求体大小和频率。
- schema 严格验证 `/cycle` 协议字段；完整游戏定义由明确的本地分析边界处理，不靠伪造 observation 根字段传输。
- 日志记录观察摘要、fingerprint、决策原因和差分，不记录浏览器隐私数据。

### 12.2 状态模型

- 可从游戏完整 maps/floors/material/source definitions 预先构建全局静态图，并用实时运行态覆盖动态事实。
- 离开楼层后可回读游戏定义或运行态；历史 observation 只用于行动恢复和审计，不充当当前 hero/cache 权威。
- 每层以引擎 floorId 标识，以坐标和 block 版本维护动态图。
- 游戏定义足以通用解释的新楼层无需暂停；只有解析失败、交互未实现或真实差分不可解释时暂停。
...
5. 对候选行动做资源差分和生存约束。
6. 在游戏完整地图模型与实时动态状态上做有限深度搜索和支配剪枝。
7. 自动处理普通路线比较、零伤怪、已知门、已知资源包和已知楼层返回。
8. 返回一个原子行动、精确 guard、expected_delta 和中文决策原因。

不得把外部攻略、标准路线、通关录像或其他人为针对性解法编码进启发式；游戏自身隐藏/未来地图和未到达 block 定义允许作为策略输入。
...
| `UNKNOWN_FLOOR` | 完整游戏定义仍无法建立该 floorId 的合法模型 | `FLOOR_MODEL_MISSING` |
...
- 直接赋值、更新或删除 hero、map/block、monster、event、save 或其他 runtime 权威对象。
- 通过 destructuring alias、`Object.assign/defineProperty`、`Reflect.set/deleteProperty`、`call/apply/bind` 等方式绕过直接 mutation 门禁。
- userscript metadata 放宽到非目标页面、非 `127.0.0.1` connect，或生成物包含未批准的运行端点。

合法正例必须覆盖 `core.floors`、完整 maps、`material`、源码定义名和存档结构只读分析，以及正常引擎行动接口调用。

### 15.2 观察测试

- 准确采集 HP、攻防、金币、经验、钥匙、floorId、坐标和方向。
- 当前执行 observation 只序列化当前动态地图仍存在且未 disable 的 blocks；策略侧可另行读取任意地图定义。
- 只对当前可见怪物坐标调用敌人接口。
- block 排序稳定，fingerprint 对等价对象一致。
- full runtime/source definitions fixture 必须可读；同时确认构建物没有把数据发往非 localhost 端点。
...
首次合法采集后，可另存一份脱敏的真实当前层 fixture，但必须确认只含 Protocol observation 字段；完整游戏定义另走明确的本地策略输入。
...
| 用户验收条件 | 设计控制 | 主要证据 |
| --- | --- | --- |
| 不使用外部攻略/路线/录像 | 来源声明 + 代码审查 + QA provenance | 策略边界文档、QA 证据 |
| 准确读取角色和位置 | engine-adapter 同步现场采集 | fake-core + 首次现场核对 |
...
| 完整游戏数据可读且不可直接篡改 | adapter 职责隔离 + engine API allowlist + actual-source 审计 + full-cycle 写入 instrumentation | production inventory + 动态零越界写；Acorn 仅作支持子集 lint |
...
| 游戏定义中的剧情或商店仍无法安全执行 | `UNKNOWN_FLOOR` / `UNSUPPORTED_INTERACTION` 暂停取证 |
