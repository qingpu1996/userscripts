...
离线实现不会伪装成全游戏穷举求解器。当前规划是“持久世界上下文 + 当前可达 frontier 的预算搜索”；商店、复杂选择、剧情和真实存档分支仍会暂停。本轮零钥匙省略修复使用 fake core 和隔离 journal 证明恢复语义，不操作真实浏览器或现场状态目录；现场复核由主会话在独立验收后统一安排，见 [QA 证据](qa/README.md)。
