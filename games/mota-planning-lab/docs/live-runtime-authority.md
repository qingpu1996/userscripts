...
游戏页面中的当前 JS 运行态是人物面板、钥匙、当前位置、忙碌状态和当前动态 blocks 的执行结算权威。本地服务不会维护一份可覆盖现场的“当前角色”镜像：每轮决策、行动前 guard 和行动后结算都重新采集当前运行态。与此同时，完整 `core.floors`、maps、`material`、源码定义和存档结构都是合法的只读策略输入，可用于预建地图、资源和机制模型。

```text
同步采集当前运行态
  -> 发送一份不可变 LiveObservation
  -> 当前 observation + 完整游戏定义/地图事实实时规划
  -> 持久化 pending 行动身份
  -> 行动 API 前再次同步采集并核对
  -> 正常游戏 API 执行一个原子边界
...
策略层可以读取完整 runtime/source definitions，包括未到达地图、素材、物品、怪物、事件及存档结构。它们用于通用解析静态语义和候选路线，不受旧 current-map 白名单限制。读取必须保持只读；定义中的 hero 或地图快照不能覆盖行动前 fresh guard，也不能代替执行后的真实差分。
...
服务结算当前动作时从请求中的 observation 取得这些当前动态值。浏览器 controller 的 `currentObservation` 只是进程内的本轮不可变引用，下一轮会被新采集替换，不是持久缓存；静态定义可参与策略，但不能覆盖动态现场。

### 2. 历史执行观察事实

离开地图后保留进入时观察到的 revision，作为动态执行历史：
