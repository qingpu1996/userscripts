...
未知出口始终 opaque：扫描只评价“执行一次出口边界”，不假设出口后仍位于原地图，也不评价其后资源。已验证出口必须能唯一关联同 session 的最新合法目标 snapshot，且 floorId、topology、dimensions 与 map-instance 元数据一致，landing 坐标有效；签发时 expected delta 精确绑定目标 `map_instance_id + floor_id`。同一出口出现多个冲突目标时暂停审计，数据不完整则保持 opaque，绝不任选其一。显示楼层数字只作遍历顺序提示。verified edge 本身不因“尚未由扫描状态机走过”而重新变成扫描目标；restart 后仍由持久 transition 身份判为已发现。
...
模拟非战斗边界后必须移除/更新该占用并重算可达性，不能将所有候选当作任意排列。此类搜索可用历史几何比较第一步，但不能借旧怪物战果给路径加分。未知换图分支在出口处终止；已验证换图才可进入目标 snapshot 继续搜索。
