...
安装时使用 `dist/mota-planning-lab.user.js`；受控测试可直接注入 `dist/mota-planning-lab.direct-mount.js`。Stage1 的 `main.js` 强制 `shadowOnly`，收到 `execute` 也会暂停而不会触发游戏移动或菜单 API；不得用于真实存档自动驾驶。
