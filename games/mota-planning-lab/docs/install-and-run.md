...
旧 Python 后端已删除。Stage2B Rust shadow runtime 只返回当前楼层边界及即时成本的只读分析，不会产生可执行动作。
...
安装时使用 `dist/mota-planning-lab.user.js`；受控测试可直接注入 `dist/mota-planning-lab.direct-mount.js`。启动时会最多等待 10 秒让游戏创建 `core.status`，页面卸载或超时则安全暂停。`main.js` 强制 `shadowOnly`，收到 `execute` 也会暂停而不会触发游戏移动或菜单 API；Stage2B 分析不代表全局最优，不得用于真实存档自动驾驶。

运行态只保留进程内存：JS 采集当前游戏状态，Rust 输出当前楼层只读分析，Stage2B JS 只展示该结果。没有旧 Python、状态恢复、后台队列、数据库或日志文件；旁路复盘日志尚未实现。
