...
> Protocol v2 中，block 标签仍按 `id + cls + trigger` 审计；世界身份改为 session 内 `map_instance_id`，不能用显示楼层号合并地图。对新 topology revision 的证据必须保留 map instance、topology fingerprint 和 dimensions。标签可以引用游戏自身完整定义，但不能引用外部攻略、标准路线或通关录像。

人工标签可来自服务保存的 pause evidence 或游戏自身 floors/maps/material/source definitions。不要根据外部攻略、记忆中的标准路线或通关录像预填知识，也不要直接编辑游戏权威状态。
...
当完整游戏定义解析仍产生 `UNKNOWN_FLOOR` 时，选择对应 evidence 文件：
...
命令会先用严格 CycleRequest schema 验证输入，因此不能把完整 maps 塞进 `/cycle` observation 的额外字段；完整游戏定义应由明确的本地分析输入读取，这不是数据禁令。

## 审计和回退

标签文件是 JSON，写入使用原子替换。每项带 `source` 和 `version`；无 trigger 的普通墙等标签会写成显式 `"trigger": null`，这是 identity 的必填可空部分，不能删除。序列化只省略真正未设置的 optional 字段，CLI 写入后会通过同一知识模型重新加载验证。修改前可复制用户知识目录做备份；可以提交通用解析规则，但不得提交来自外部攻略/标准路线/录像的针对性解法数据。
