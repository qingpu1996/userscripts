...
当前生产实现基线为 `9969126b6e6702709693298b19193f54e6747002`；Phase 5D Commit 3 只补充文档与 QA 归档，不改变生产搜索语义。最后两次生产语义变更分别是 `e04c5b00ed6d4471513d19a133e4550078167c4d`（认证攻防宝石的 Phase A root policy）与 `9969126`（initial immediate safe action）。Phase 4B `c24d09d` 仍提供 Phase A static region/portal graph；Phase 5A/5A.1 仍仅是 profile-only 资格/机会证据。Phase 4B 归档的 1.3538% A/B 结果仍只属中性，不构成性能提升主张。
...
```mermaid
flowchart TD
  A[解析 solver_model 与初始 hero] --> B[建立 ConsumedBits、inventory、shop counts、flags\n并编译 CombatGemCertificate]
  B --> C[初始 Phase A view 一次\nsafe gate failure => exact cell BFS]
  C --> D{same-floor certified gem?}
  D -- 是 --> Z[unproven immediate read-only suggestion\n不创建 Phase A store/queue 或 Phase B]
  D -- 否 --> E[收集 boundary、shops、reachable terminal]
  E --> F[Phase A VecDeque PhaseAWorkItem\naccepted source + compact action ref]
  F --> G[pop_front；source 即使 stale 仍可寻址]
  G --> H[materialize；成功后只计算一次 view]
  H --> I[canonical representative]
  I --> J[StructuralNode interner + 8 维 Pareto frontier]
  J --> K[标记被支配 label 为 stale]
  K --> L[NumericObjective]
  L --> M{terminal?}
  M -- 否 --> E
  M -- 是 --> N[只更新数值目标并继续搜索]
  N --> O{Phase 1 队列耗尽?}
  O -- 是 --> P[Phase 2：固定数值目标，字典序路线见证]
```
...
Phase A 的 action ref 只有带 tag 的稳定 block/shop 索引、shop choice、shop tile 索引和相邻 cell，不能保留 navigation、String 或完整状态。initial `view` 只计算一次：若其中 local-origin、same-floor boundary 有认证攻防宝石，则不创建 Phase A store/queue 或 Phase B，直接返回一个明确 `unproven` 的 read-only immediate suggestion；若未命中，复用该 state/view canonicalize/Pareto 并进入 Phase A。以后每个 FIFO work item 在真正 pop 时才 materialize。成功后复用这次 view 的 representative 做 canonicalize/Pareto、终局判断和后继入队，拒绝后立即丢弃该 view；代表 BFS 热路径为 0。

`CombatGemCertificate` 只在 parse 后编译一次，按 block index 返回 fail-closed 的 red/blue gem 认证；它要求准确的 typed `+3` 形状、独立普通消费 slot、无动态同格 blocker、无其他 rule 对 slot 的读/写/reactivate/replace/unknown 依赖，以及有限且不会被后续支持规则降低/覆盖的攻防数值域。具体数值域是：attack/defense 必须是精确的非负整数且不超过 `2^53`；world envelope 使用 checked `u128` 累加所有一次性奖励、商店上界和 audited 事件，再检查 `+3` 后仍在 exact 范围。Fairy 这类非交换变换只有在严格的 operation-count 与 binary64 rounding envelope 成立时才允许跨越；否则把其 required-consumed slot 编译进 mask，只有该 slot 已消费才认证。hot path 只做 indexed certificate、finite/exact、`+3`、upper-bound 与 required-consumed-mask 检查，不重扫 world。新增 typed enum/rule 必须穷尽审计，否则整个认证 fail closed。对每个已接受 source，若其 `view.boundaries` 的稳定顺序中第一个认证 gem 存在，Phase A 仅入队该一个**既有** work item，压制其余 boundary 和 shop choice；不产生 batch、wave、额外 view、专用队列或 Pareto 前处理。无认证 gem 时原有顺序不变。gem 后继仍走正常 materialize/view/interner/frontier 路径；Phase B 不使用强制顺序，仍在原动作图上找 canonical witness。新 label 支配旧 label 时旧 label 虽标记 stale，已入队 work item 仍可从 append-only arena 取回 source 并保持 FIFO 语义。第 `max_states` 个 accepted/expanded label 计入预算后，queue 空返回 Complete，否则返回 BudgetExhausted；不会 materialize 下一个 candidate。终局只更新 `NumericObjective`。候选包括所有可达 boundary（door、enemy、resource、event、战略 transition）以及可达受限商店的每个 choice；当前楼层的 `current_floor_immediate` 分析是同一响应中的独立即时 BFS，最多返回 256 个候选。
...
Phase 5D 是一个不同且已交付的窄策略：不复活 Phase 5B closure，也不把所有 resource 当作单调。它共享上述 `CombatGemCertificate`，但把 root policy 与 immediate response 严格分开。内部 root policy 仍是 Phase A 的全局数值证明搜索：认证 gem free、只增加 attack 或 defense、不会关闭通路或改变条件，且其 slot/算术依赖已被审计；所以可将任何最优路线中的该 gem commute 到 source 前，不取 gem 的路线不会严格更优。官方 50k 仅允许该策略相对同一最终二进制 gate-off 回退不超过 5%，而 same-depth/work profile 用于检查它是否删掉延迟 gem 排列。

Immediate 只在**真实 initial view** 的同层 local-origin boundary 命中认证 gem 时触发。它经 production materializer 生成现有 `RouteStep`，返回 `decision_mode: certified_immediate_action`、`proof: unproven`、`route: null` 和 `explored_states: 0`，并明确 reason 为 `deferred_for_certified_immediate_combat_gem`。这不是完整通关或终局路线证明；本轮完整 proof 被延期，下一 cycle 必须重新采集 observation。它不调用 adapter、executor、键盘、鼠标或页面 API，也不持久化搜索。若证书与 materializer 矛盾，分析 fail closed 为 unsupported，不能悄悄回落到全局搜索。完整证据见 [`Phase 5D certified combat-gem QA`](qa/phase5d-certified-combat-gem-policy-2026-07-20/README.md)。
...
Phase 2 的临时节点保存 block/shop 动作及 navigation segment，以及与输出 steps 等长的 typed key；Phase 1 不保留这些数据。响应中的 route 只读描述字段可按 step 类型变化，例如包含 `step_kind`、`floor_id`、坐标和详情；block step 还带 `block_id`，shop step 还带 `shop_id`/`choice_id`。协议递归拒绝 `action`、`operation`、`guard` 等可执行字段。认证 immediate response 的 `first_suggestion` 也复用该只读 step 格式，但它没有 route，且 UI 明示“安全单步建议、全局证明延期”。当前 Rust 不自动操作页面、不保存路线到磁盘；下一轮由 JS 重新采集 observation。浏览器 journal 可保存会话/恢复元数据，但不承载 Rust 搜索状态。未来执行层若获得另行授权，最多消费一个 immediate suggestion，之后必须重新 observation，不能继续复用旧 view/state/suggestion。
...
Phase 5D final `9969126` 用独立 archive build 的 7+7+7 official A/B/C 重新测量：`6fa0` / final-policy-off / final-policy-on median 为 `1196.425 / 1239.973 / 1248.365 ms`。A→B default-off overhead 为 `+3.640%`；C 相对同二进制 B 回退 `+0.6768%`，低于 5% 策略门槛；C max RSS `48,152,576 B`，低于 55 MB。三 lane 的 HTTP canonical response 相同且均为 50k budget-unproven。profile 中 root policy 将 pending `281,991 → 183,675`，认证 roots `2,974`，压制 ordinary boundary/shop actions `31,123 / 8,922`；固定 depth 8/12/16 的 B/C 则显著减少 popped/view 工作，固定 200k work item 时 C reach depth 17、B depth 15。它改变 accepted semantic trace 是预期调度结果，不是外部响应差异。另以同一 final binary、official request、internal policy on 且 immediate 环境变量保持 production default 的独立 warmup + one profile 复核 certificate miss：initial view/store 为 `1/1`、immediate hit/skip 为 `0/0`，accepted trace `26bda7fdd33330f3`、accepted/work/views 为 `50,000/408,244/177,868`，与 C lane 一致。initial immediate fixture 7+7 从 full global `1.877 ms / 41 explored` 变为 `0.541 ms / 0 explored`，profile 为 one initial view、zero store/Phase A/Phase B、one hit/skip。所有数字绑定精确源/二进制/request hash，见 [`Phase 5D QA`](qa/phase5d-certified-combat-gem-policy-2026-07-20/README.md)；profile/test depth/work knobs 均只供诊断，不进入默认协议。
...
- Phase 5D official input 同样是 `PhaseB=0` 的 budget-exhausted 请求；root-policy global-proof 语义由小世界/route replay tests 与 certificate 约束支持，但还不是任意真实存档的性能或 Phase B 后续成本证明。immediate fixture 仅证明单步 safe action 和 deferred response contract，绝不是终局 proof。
...
| 认证 gem policy / immediate | 同上：`CombatGemCertificate`、`enqueue_phase_a_actions`、`global_analysis_with_stats`、`PreparedPhaseAInitial` |
