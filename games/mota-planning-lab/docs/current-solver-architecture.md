...
flowchart TD
  ...
  D --> E[Phase 1 VecDeque LabelId\n只存已接受 label]
  E --> F[pop_front；跳过 stale label]
  F --> G[从 StructuralNode 重建临时状态并 materialize 动作]
...
Phase 1 是 FIFO 的 bounded label-setting search：`VecDeque<LabelId>` 按已接受 label 的入队顺序取出，不按启发式优先级排序。队列不保存 navigation 或完整候选；pop 后从 `StructuralNode` 重建临时 `SolverState`、读取当前 connectivity view，并逐个 materialize block/shop 动作。无效钥匙、资源、战斗、商店或未支持事件在 materialize 阶段返回 `None`，不会产生后继。动作资源可行性始终从 label 的实时资源检查，未写入结构缓存。

候选 materialize 后先计算只包含连通分量/可逆换层闭包的轻量 representative，再以该 canonical 位置 intern `StructuralNode` 并由八维 Pareto frontier 接受或拒绝 label；它不为入队构造完整 `view`。label 出队时计算一次完整 `view`，直接用于动作展开。轻量 representative 与完整 view 使用相同的闭包和顺序规则，测试会逐一比较二者的 representative。新 label 支配旧 label 时会标记旧 label stale；旧 ID 不回收，后续 pop 跳过它。终局只更新 `NumericObjective`。候选包括所有可达 boundary（door、enemy、resource、event、战略 transition）以及可达受限商店的每个 choice；当前楼层的 `current_floor_immediate` 分析是同一响应中的独立即时 BFS，最多返回 256 个候选。
...
Phase 1 队列耗尽后，才启动独立、同预算上限的 Phase 2。Phase 1 由 `run_numeric_proof` 独占 label queue、结构/label arena 与 Pareto frontier；函数返回的 `PhaseAResult` 只含数值目标、完成状态和 explored 数，因而这些大对象已确定性释放，不能与 Phase 2 的 witness 队列叠加。Phase 2 将数值目标固定为 Phase 1 的最优三元组，重新计算 connectivity/navigation，并以 typed `RouteStepKey` 的最小堆展开。
...
2026-07-18 对 `4a7ad6c` 与 StructuralNode/ResourceLabel 工作树做了同一 50k HTTP 请求的 release A/B：每版预热一次，固定交错 5 个正式样本，测纯 POST wall time 和每 10ms 采样的服务 PID RSS。10 个响应均为 HTTP 200、`explored_states=50000`、`proof=unproven`、`reason=search_budget_exhausted`，且 Phase 2 未进入；正确移除 `shadow.cycle` 后，两版每次对应运行的响应 hash 相同。A 的中位 POST 为 `2049.516 ms`、最大采样 RSS 为 `138,706,944 B`；B 分别为 `3373.533 ms` 与 `83,755,008 B`，即 RSS 降 `54,951,936 B`（39.6%），但中位耗时增 `1324.017 ms`（64.6%）。源码/二进制/request hash 与逐样本原始结果记录在本机一次性 `/private/tmp/mota-structural-label-rebuild-ab.json`，不是仓库运行时依赖。

这个性能数据只代表一个固定、Phase B 为 0 的 observation，不是所有存档的 SLA。候选不再为 canonical representative 计算第二次完整 `ConnectivityIndex::view`：入队前只计算不产生 action/template/navigation 容器的轻量 representative，唯一完整 view 留给出队后的动作展开。本阶段没有添加 action-template/connectivity 缓存；Phase 2 在复杂且最终 `proven` 的真实世界上的额外开销也尚未实测。

本次原始 QA 证据归档见 [`StructuralLabel rebuild QA`](qa/structural-label-rebuild-2026-07-18/README.md)；仓库归档，仅供审阅，不是运行时依赖。

搜索剖析仅在 `/tmp` 的 instrumented 副本中执行一次，未进入交付源码：该样例有 394,248 次 representative BFS（合计 2.087 s，median 5.167 μs）、50,000 次完整 view BFS（合计 0.312 s，median 6.083 μs）、678,693 次 candidate materialization（合计 0.174 s）；Pareto 接受/拒绝分别为 108,376/285,872，pop/stale-pop 为 73,502/23,501。证据在 `/private/tmp/mota-structural-label-search-profile-result.json`。

历史 profile 中出现的“frontier 约 40 MiB、route/navigation 等分项”来自更早的容器快照或 bitset 之前版本；当前没有把这些旧分项相加当作 `af5b511` 的精确结构占用。已保留的优化包括 COW 状态、动态 mutable slots、Phase 1 移除父链和 route arena、interned `StructuralNode`/`ResourceLabel`、LabelId queue、可逆联通索引和 `ConsumedBits`；Phase 2 只在当前请求内保存临时 route/navigation witness。曾经的 eager reversible-region 搜索已在 `bf33f0d` 回滚。
...
- Phase A 的 label queue、结构/label arena 和 Pareto frontier 仍会随动作组合增长；Phase A 结束后会释放它们，但 Phase 2 的 route/navigation witness 当前仍为完整 `Vec` 深复制，复杂 proven 输入尚未使用 persistent parent/trie/LCP 压缩，也没有最终每类结构的稳定内存配额。
- 50k A/B 仅覆盖一个 `PhaseB=0` 的未证明请求；不同存档的时间、RSS 和 frontier 形状仍须单独测量。该样例的 RSS 已明显下降，但 64.6% 的中位耗时回归不满足性能验收，不能据此将本实现视为性能可交付替代。
