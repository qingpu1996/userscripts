...
已验收事实基线为 `c24d09d32d8b19d8daa30e07d8d2ee3dd6b967ac`（Phase 4B closeout）。该提交包含 `299786b564e9d75b1db52b3eeab65451736a50ad` 的 static region/portal graph、共享 `BlockPassability` 分类、profile-only accepted semantic trace，以及与源码哈希精确绑定的 QA 归档。归档的 1.3538% A/B 结果仍只属中性，不构成性能提升主张。
...
Phase 5A 只建立资源闭包资格契约和 profile-only 机会测量，**没有启用 closure**。当前唯一 eligible 子类是 typed resource 的恒等 delta：初始 active、有自身 consumed slot，所有加数为零、浮点乘数为一、inventory 为空，除正常 `false -> true` consumed 写外无结构/资源副作用；现有 connectivity representative 还必须消去动作邻接位置差异。任何真实数值增益都因 checked overflow 或浮点 fault/order 域排除，门、敌人、商店、事件、transition、opaque/unknown、level/inventory/flags/cross-resource 效果也全部排除。完整 oracle、profile 字段和 Phase 5B 门槛见 [`Phase 5A closure contract`](qa/phase5a-closure-contract-2026-07-18/README.md)。
