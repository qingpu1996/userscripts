...
同一次采集在读取 map/blocks/怪物前后核对当前 floor、hero/keys/位置和最小 busy 围栏；不一致则整份丢弃，不会混合两个时刻。钥匙必须来自已登记完整布局；字段缺失或冲突不会用零兜底。
...
- 真实 `hero.items.tools.*Key` 形状 fixture、key layout 冲突、torn snapshot、历史资源污染和重访 revision 均有 fail-closed 回归。
